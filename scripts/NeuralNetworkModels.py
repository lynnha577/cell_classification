...
class mlp_voltage_predictor(nn.Module):
    ...
    def forward(self, x):
        x = self.layer1(x) # input goes through first layer
...
class LSTM_voltage_predictor(nn.Module):
    # sets up layers (2) and activation function (ReLU)
    def __init__(self, input_size, output_size):
        super().__init__()
        #self.layer1 = nn.Linear(input_size, 32)
    ...
    def forward(self, x):
        ...
        x = self.activationFunction2(x) # ReLU is activated
        print(x.size())
        x, _= self.layer2(x) # data goes through second layer
        x = self.activationFunction2(x) # ReLU is activated 
